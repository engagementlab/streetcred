class ParticipantsController < ApplicationController
  ...
  def show
    ...
    gon.markers = @participant.actions.where(:campaign_ids.exists => true).reject {|x| x.latitude.blank? || x.longitude.blank?}.collect {|x| {type: 'Feature', geometry: {type: 'Point', coordinates: [x.longitude, x.latitude]}, properties: { title: x.user.try(:display_name), description: "#{x.action_type.try(:channel).try(:name)}<br />#{x.action_type.try(:name)}<br />#{ActionController::Base.helpers.link_to(x.created_at.strftime('%m/%d/%Y'), participant_action_path(x.user, x))}", 'marker-size' => 'small', 'marker-color' => '#ff502d'}}}
