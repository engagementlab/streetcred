class ParticipantsController < ApplicationController
  ...
  def show
    ...
    @actions_with_text = @participant.actions.desc(:created_at).limit(5).select {|x| x.body.present? || x.subject.present? }
    @actions_with_photos = @participant.actions.or({:image_url.exists => true}, {:photo_url.exists => true}).desc(:created_at).limit(5)
