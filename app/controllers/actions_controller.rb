class ActionsController < ApplicationController
  ...
  def show
    ...
    if @action.latitude.present? && @action.longitude.present?
      gon.markers = [{type: 'Feature', geometry: {type: 'Point', coordinates: [@action.longitude, @action.latitude]}, properties: { title: @action.user.try(:display_name), description: "#{@action.action_type.try(:channel).try(:name)}<br />#{@action.action_type.try(:name)}<br />#{view_context.formatted_date(@action.created_at)}", 'marker-size' => 'small', 'marker-color' => '#ff502d'}}]
    end
